...
def process_grid_data(grid_data):
    ...
    if marker_count == 0:
        ...
        # Single marker is terminal: interpret 8 bytes as two floats (position, BPM).
        ...
        # Multiple markers: all but the last are non-terminal and the final one is terminal.
...
def get_beatgrid(file_path):
    ...
    # We'll process only the first beatgrid group.
    for entry in beatgrid_entries:
        ...
        # Discard the first part (e.g. "application/octet-stream").
        data_part = parts[1]
        ...
        for marker in markers:
            if marker["type"] == "terminal":
                terminal = { "position": marker["position"], "bpm": marker["bpm"] }
            else:
                non_terminal.append({
        ...
        return result
    # If no valid beatgrid group is processed:
